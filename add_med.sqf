...
{	
	...
	if (_x == (leader group _x)) then {(uniformContainer _x) additemcargoGlobal ["ACE_MapTools", 1];};
		...
		(backpackContainer _x) additemcargoGlobal ["ACE_elasticBandage",9];
		(backpackContainer _x) additemcargoGlobal ["ACE_packingBandage",9];
		...
		(backpackContainer _x) additemcargoGlobal ["ACE_salineIV",4];
